<?php

$REX['ADDON']['seo42']['settings']['rewriter'] = true;
$REX['ADDON']['seo42']['settings']['url_ending'] = '.html';
$REX['ADDON']['seo42']['settings']['hide_langslug'] = 0;
$REX['ADDON']['seo42']['settings']['homeurl'] = 2;
$REX['ADDON']['seo42']['settings']['homelang'] = 0;
$REX['ADDON']['seo42']['settings']['no_double_content_redirects'] = 1;
$REX['ADDON']['seo42']['settings']['no_double_content_redirects_availability'] = 1;
$REX['ADDON']['seo42']['settings']['auto_redirects'] = 0;
$REX['ADDON']['seo42']['settings']['smart_redirects'] = true;
$REX['ADDON']['seo42']['settings']['redirects_allow_regex'] = false;
$REX['ADDON']['seo42']['settings']['css_dir'] = 'lib/project/css/';
$REX['ADDON']['seo42']['settings']['js_dir'] = 'lib/project/js/';
$REX['ADDON']['seo42']['settings']['images_dir'] = 'lib/project/images/';
$REX['ADDON']['seo42']['settings']['icons_dir'] = 'lib/project/icons/';
$REX['ADDON']['seo42']['settings']['seo_friendly_image_manager_urls'] = true;
$REX['ADDON']['seo42']['settings']['full_urls'] = false;
$REX['ADDON']['seo42']['settings']['allow_article_id'] = false;
$REX['ADDON']['seo42']['settings']['ignore_root_cats'] = false;
$REX['ADDON']['seo42']['settings']['url_whitespace_replace'] = '-';
$REX['ADDON']['seo42']['settings']['url_start'] = '/';
$REX['ADDON']['seo42']['settings']['url_start_subdir'] = './';
$REX['ADDON']['seo42']['settings']['remove_root_cats_for_categories'] = array (
);
$REX['ADDON']['seo42']['settings']['no_url_for_categories'] = array (
);
$REX['ADDON']['seo42']['settings']['include_query_params'] = true;
$REX['ADDON']['seo42']['settings']['ignore_query_params'] = array (
);
$REX['ADDON']['seo42']['settings']['force_download_for_filetypes'] = array (
);
$REX['ADDON']['seo42']['settings']['send_header_x_ua_compatible'] = true;
$REX['ADDON']['seo42']['settings']['fix_image_manager_cache_control_header'] = false;
$REX['ADDON']['seo42']['settings']['static_sitemap_priority'] = true;
$REX['ADDON']['seo42']['settings']['no_robots_txt_auto_disallow'] = true;
$REX['ADDON']['seo42']['settings']['robots'] = '';
$REX['ADDON']['seo42']['settings']['google_site_verification_filename'] = '';
$REX['ADDON']['seo42']['settings']['robots_follow_flag'] = 'follow';
$REX['ADDON']['seo42']['settings']['robots_archive_flag'] = 'noarchive';
$REX['ADDON']['seo42']['settings']['title_delimiter'] = '-';
$REX['ADDON']['seo42']['settings']['seopage'] = true;
$REX['ADDON']['seo42']['settings']['title_preview'] = true;
$REX['ADDON']['seo42']['settings']['no_prefix_checkbox'] = false;
$REX['ADDON']['seo42']['settings']['custom_canonical_url'] = false;
$REX['ADDON']['seo42']['settings']['noindex_checkbox'] = false;
$REX['ADDON']['seo42']['settings']['urlpage'] = true;
$REX['ADDON']['seo42']['settings']['all_url_types'] = true;
$REX['ADDON']['seo42']['settings']['pagerank_checker'] = true;
$REX['ADDON']['seo42']['settings']['redirects'] = true;
$REX['ADDON']['seo42']['settings']['one_page_mode'] = false;
$REX['ADDON']['seo42']['settings']['pagerank_checker_unlock'] = false;
$REX['ADDON']['seo42']['settings']['global_special_chars'] = '';
$REX['ADDON']['seo42']['settings']['global_special_chars_rewrite'] = '';
$REX['ADDON']['seo42']['settings']['urlencode_whitespace_replace'] = '_';
$REX['ADDON']['seo42']['settings']['urlencode_lowercase'] = false;
$REX['ADDON']['seo42']['settings']['lang'] = array (
  0 => 
  array (
    'code' => 'de',
    'original_name' => 'deutsch',
    'rewrite_mode' => 0,
    'special_chars' => 'Ä|ä|Ö|ö|Ü|ü|ß|&',
    'special_chars_rewrite' => 'Ae|ae|Oe|oe|Ue|ue|ss|und',
    'inherit_from_clang' => 0,
  ),
);
